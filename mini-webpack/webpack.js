...
function bundle(graph) {
  ...
  graph.forEach(mod => {
    modules += `${mod.id}: [
      ...
      ${JSON.stringify(mod.mapping, null, 2)},
    ],`
  })
  const result = `
    (function(modules) {
...
// 项目的入口文件
const graph = createGraph(pathResolve('./tests/index.js'))
const result = bundle(graph)

// 移除打包文件
if (fs.existsSync(pathResolve('./dist'))) {
  fs.rmdirSync(pathResolve('./dist'), { recursive: true })
}

// 创建dist目录，将打包的内容写入main.js中
fs.mkdir(pathResolve('./dist'), err => {
  if (!err)
    fs.writeFile(pathResolve('dist/main.js'), result, err1 => {
